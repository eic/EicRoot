...
void analysis()
{
  ...
  // This assumes single-track events of course (a typical case for a particle gun);
  ...
  char cut[1024];
  // Allow to lose a couple of degrees of freedom compared to the max (assume standard) case;
  ...
  // Fitting is performed in 3 iteration, the last one in a +/-5*sigma range;
  TH1D *dp1 = new TH1D("dp1", "dp1", 100, -20.0, 20.0);
  cbmsim->Project("dp1", expression, cut);
  TF1 *fq1 = new TF1("fq1", "gaus(0)",    -20.0, 20.0);
  // Be aware, that par[2] (sigma) may require some tuning here;
