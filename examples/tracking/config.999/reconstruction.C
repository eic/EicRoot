...
void reconstruction()
{
  ...
  gSystem->Load("/DATA00/ayk/FAIRROOT.FRESH/FairSoft-jul15p1/build/lib/libgeant321.so");
  EicHtcTask *htc = new EicHtcTask(idealTracker, TrKalmanFilter::WithField);
  htc->SetTrackOutBranchName("EicIdealGenTrack");
  htc->SetHitOutputCoordinateScaleXY(1E1);  
  htc->SetHitOutputSlopeScale       (1E0);
  htc->SetHitOutputResidualScaleXY  (1E4);
  htc->SetParticleHypothesis("pion",    10.0);  
  fRun->AddTask(htc);
