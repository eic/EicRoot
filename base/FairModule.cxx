...
FairModule::FairModule(const char* Name, const char* title ,Bool_t Active)
  :TNamed(Name, title),
   ...
   flGeoPar(0),
   kGeoSaved(kFALSE),
   fRootMaterialImportFlag(kFALSE),
...
void FairModule::AssignMediumAtImport(TGeoVolume* v)
{
  ...
  if(med1 && strcmp(med1->GetName(), "dummy")) {
    ...
    // FairGeoBuilder does not distinguish material and media names (I guess); STAR
    // geometry database does (say media CAVE_AIR is done from material AIR); so 
    // switch to using media names here and below;
#if _OLD_
...
#else
      FairGeoMedium* FairMedium=Media->getMedium(med1->GetName());
#endif
      if (!FairMedium) {
