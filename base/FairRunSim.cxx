...
//  fOutFile=fRootManager->OpenOutFile(fOutname);
  ...
  if (!fApp) fApp= new FairMCApplication("Fair","The Fair VMC App",ListOfModules, MatFname);
...
void FairRunSim::Run(Int_t NEvents, Int_t NotUsed)
{
  ...
  // Help tutorials/designer/calorimeter/simulation.C to call exit() at the end without 
  // crashing; this is related to how fEicMoCaPointCollection variable is declared for 
  // ROOT streaming; why the problem occurs in a first place is still a mistery;
  if (GetOutputFile()) GetOutputFile()->Close();
}
//_____________________________________________________________________________
